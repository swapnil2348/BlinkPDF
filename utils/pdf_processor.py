...
class PDFProcessor:
    ...
    def process(
        ...
        if slug == "compress-pdf":
            level = options.get("compression_level", "medium")
        ...
        if slug == "pdf-to-image":
            ...
            dpi = int(options.get("output_dpi", "150"))
        ...
        if slug == "crop-pdf":
            # Expect margins in points or empty for no-op
        ...
        if slug == "fill-forms":
            # Expect JSON of {field_name: value} in form_data_json
    ...
    def compress_pdf(self, path: str, level: str) -> Tuple[str, str, str]:
        """
        Simple compression using PyMuPDF: garbage collection + deflate.
        Level is kept for future tuning, but we already do real compression.
        """
        doc = fitz.open(path)
        out_path = self._tmp_file(".pdf")

        # Basic garbage / compression
        doc.save(
            out_path,
            garbage=4,
            deflate=True,
    ...
    def optimize_pdf(self, path: str) -> Tuple[str, str, str]:
        """
        Aggressive optimize similar to compress, can be tuned separately.
        """
        doc = fitz.open(path)
        out_path = self._tmp_file(".pdf")
        doc.save(
    ...
    def word_to_pdf(self, path: str) -> Tuple[str, str, str]:
        ...
        into a PDF using ReportLab. Formatting/layout from Word is not preserved,
    ...
    def pdf_to_excel(self, path: str) -> Tuple[str, str, str]:
        ...
        into successive rows. This is a real export, but NOT a perfect table detector.
        """
        doc = fitz.open(path)
    ...
    def pdf_to_powerpoint(self, path: str) -> Tuple[str, str, str]:
        ...
        # Use default slide size, fit image inside.
    ...
    def powerpoint_to_pdf(self, path: str) -> Tuple[str, str, str]:
        """
        PPTX->PDF: WITHOUT a rendering engine we cannot get slide screenshots.
        Instead we export the text content of each slide into a PDF using ReportLab.
        This is still a real conversion but not layout-accurate.
        """
    ...
    def ocr_pdf(self, path: str, lang: str = "eng") -> Tuple[str, str, str]:
        ...
        pytesseract will raise an error which we propagate as a clear message.
        """
        try:
            doc = fitz.open(path)
            out_path = self._tmp_file(".pdf")
    ...
    def extract_images(self, path: str) -> Tuple[str, str, str]:
        ...
        for page_index in range(len(doc)):
            for img_index, img in enumerate(doc.get_page_images(page_index), start=1):
                ...
                img_ext = base["ext"]
                img_path = os.path.join(tmpdir, f"page{page_index+1}_img{img_index}.{img_ext}")
    ...
    def edit_metadata(self, path: str, meta: Dict[str, str]) -> Tuple[str, str, str]:
        ...
        cleaned = {k: v for k, v in meta.items() if v and k in {
